...
def get_request_args(rspec: dict, test_block_config: TestConfig) -> dict:
    ...
    if len(in_request) > 1:
        ...
        if set(in_request) != {"data", "files"}:
            raise exceptions.BadSchemaError("Can only specify one type of request data in HTTP request (tried to " "send {})".format(" and ".join(in_request)))
    ...
    if "files" in rspec:
        if content_header:
            logger.warning("Tried to specify a content-type header while sending multipart files - this will be ignored")
            rspec["headers"] = {i: j for i, j in normalised_headers.items() if i.lower() != "content-type"}

    fspec = format_keys(rspec, test_block_config.variables)

    if fspec["method"] not in valid_http_methods:
        raise exceptions.BadSchemaError("Unknown HTTP method {}".format(fspec["method"]))
    ...
    if filename:
        with ExitStack() as stack:
            ...
            if group_name:
                logger.warning(f"'group_name' for the 'file_body' key was specified as '{group_name}' but this will be ignored ")
...
def _read_expected_cookies(session: requests.Session, rspec: Mapping, test_block_config: TestConfig) -> Optional[dict]:
    """
    ...
    cookies_to_use = format_keys(rspec.get("cookies", None), test_block_config.variables)
    ...
    if missing:
        ...
        raise exceptions.MissingCookieError(f"Tried to use cookies '{expected}' in request but only had '{existing_cookies}' available")
    ...
    if len(extra) != len(from_extra):
        ...
        raise exceptions.DuplicateCookieError("Tried to override the value of a cookie multiple times in one request")

    overwritten = [i for i in expected if i in from_extra]

    if overwritten:
        logger.error("Duplicate cookies found in request")
        raise exceptions.DuplicateCookieError(f"Asked to use cookie {overwritten} from previous request but also redefined it as {from_extra}")
...
class AwsRequest(RestRequest):
    ...
    def __init__(self, session: requests.Session, rspec: dict, test_block_config: TestConfig) -> None:
        """Prepare request
        ...
        def prepared_request():
            ...
            with ExitStack() as stack:
                ...
                if file_body:
                    ...
                    files = get_file_arguments(self._request_args, stack, test_block_config)
                    if files:
